...
CCGreenScreenTexture::CCGreenScreenTexture() {
...
void CCGreenScreenTexture::SetupAssetManager(AAssetManager *assetManager, std::string path) {
    m_pAssetManager = assetManager;
    m_directoryPath = path;
}

void CCGreenScreenTexture::InitGL(){
...
void CCGreenScreenTexture::loadTextureResource(AAssetManager *pManager) {
    //创建纹理图片
    CCOpenGLTexture glTexture;
    m_texId_1 = glTexture.CreateTextureFromFile(pManager,"texture/green_screen.png");
    m_texId_2 = glTexture.CreateTextureFromFile(pManager,"texture/zhoushan1.png");
    mVideoWidth = glTexture.getWidth();
    mVideoHeight = glTexture.getHeight();
}

void CCGreenScreenTexture::loadShaderResource(AAssetManager *pManager) {
    //加载编译顶点着色器和片段着色器
    m_pOpenGLShader->InitShaderFromFile(pManager,
                                        "shader/greenscreen/greenScreenv.glsl",
                                        "shader/greenscreen/greenScreenf_2.glsl");
}

void CCGreenScreenTexture::setupPlaneResource() {
    ...
    const CCFloat5 planeVertexs[] = {
            {  -1.0f,  0.3f, 0.0f,  1.0, 1.0 },
            {  -1.0f, -0.3f, 0.0f,  1.0, 0.0 },
            {  1.0f, -0.3f, 0.0f, 0.0, 0.0 },
            {  1.0f,  0.3f, 0.0f,  0.0, 1.0 },
    };
...
void CCGreenScreenTexture::PaintGL() {
    //清空颜色缓冲区和深度缓冲区
...
void CCGreenScreenTexture::ResizeGL(int width, int height) {
    glViewport(0,0,width,height);
}

void CCGreenScreenTexture::Release() {
    glDeleteTextures(1,&m_texId_1);
    glDeleteTextures(1,&m_texId_2);
    CCGLSafePtrDelete(m_pCamera);//释放摄像机
    CCGLSafePtrDelete(m_pVAO);
