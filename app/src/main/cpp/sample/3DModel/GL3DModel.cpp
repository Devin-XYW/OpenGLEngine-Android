...
void GL3DModel::SetupAssetManager(AAssetManager *assetManager, std::string path) {
    m_pAssetManager = assetManager;
    asset_dir = path;
}

void GL3DModel::InitGL() {
    if(m_pModel != nullptr && m_pShader != nullptr)
        return;
    initSceneResources();
}

void GL3DModel::initSceneResources() {
    //加载含有动画的3D模型
    std::string path(DEFAULT_OGL_ASSETS_DIR);
    std::string modelPath(path + "/model/3Dmodel/model_3/Punching Bag.fbx");
...
void GL3DModel::PaintGL() {
    if(m_pModel == nullptr || m_pShader == nullptr) return;

    //首先更新动画
    //这个值为0.03的原因：首先基于1s30帧，所以则算下来，一帧0.03s
...
void GL3DModel::ResizeGL(int width, int height) {
    mScreenW = width;
    mScreenH = height;
}

void GL3DModel::Release() {
...
void GL3DModel::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    ...
    glm::mat4 Projection = glm::frustum(-ratio, ratio, -1.0f, 1.0f, 1.0f, m_pModel->GetMaxViewDistance() * 1000);
    //glm::mat4 Projection = glm::perspective(45.0f,ratio, 0.1f,100.f);

    // View matrix
    glm::mat4 View = glm::lookAt(
            glm::vec3(0, 0, m_pModel->GetMaxViewDistance() * 3.0f), // Camera is at (0,0,1), in World Space
...
void GL3DModel::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
